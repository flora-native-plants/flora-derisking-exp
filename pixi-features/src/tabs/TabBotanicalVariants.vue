...
const pwWarpAmp       = ref(0.20)   // UV warp amplitude (bigger decentres tide-lines)
const pwFbmB          = ref(0.15)   // T-field fBm weight
const pwPaperC        = ref(0.03)   // T-field paper weight
const pwBandCount     = ref(3)      // iso-band count [3, 7]
const pwEdgeWidth     = ref(1.2)    // band edge width multiplier
...
function rebuild() {
  ...
  // A = settling olive-green (deeper than last round — the muted look comes from paper-through
  // grain, not a thin wash), B = grayed warm brown (~#a07560).
  const pigA = pigmentKS([0.28, 0.42, 0.26], [0.06, 0.14, 0.07])
